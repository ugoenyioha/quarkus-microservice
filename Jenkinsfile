node {
	...
	stage('Push chart to S3') {
	    withAWS(region:'us-west-2', credentials:'spinnaker-admin-aws') {
          s3Upload(file:'quarkus-microservice-chart.tar.gz', bucket:'helm-charts-bucket-f2bba284-98d3-445b-9f04-a08c57b7d36e', path:"${JOB_NAME}/${BUILD_ID}/quarkus-microservice-chart.tar.gz")
        }
	}
	...
	stage('Push to ECR') {
		docker.withRegistry('https://951980950913.dkr.ecr.us-west-2.amazonaws.com/spinnaker', 'ecr:us-west-2:spinnaker-admin-aws') {
